...
| FR-4.5 | AQI: station-computed + city bulletin | ✅ station-computed (`GET /v1/aqi/stations`, CPCB 24h/8h-averaged default + `basis=latest` snapshot); 🟡 city bulletin backfill pending | `api.AqiController`, `aqi.AqiService`, `MeasurementRepository.trailingAverages` |
