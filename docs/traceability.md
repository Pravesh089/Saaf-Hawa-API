...
| FR-4.5 | AQI: station-computed + city bulletin | ✅ station-computed (`GET /v1/aqi/stations`, CPCB 24h/8h-averaged default + `basis=latest` snapshot + `latest-fallback`); ✅ city bulletin backfill (urbanemissions CSV, historical only — see verification-log V6/V7) | `api.AqiController`, `aqi.AqiService`, `ingest.source.urbanemissions.UrbanEmissionsAdapter` |
